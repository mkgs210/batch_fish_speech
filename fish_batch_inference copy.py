...
for response in responses:
    ...
    if hasattr(response, 'action') and response.action == "sample":
        ...
        logger.info(f"[BATCH] [{idx}] {response.text} → {wav_path} | Time for chunk: {time.time() - t_ch:.2f} seconds")
        idx += 1
    elif hasattr(response, "action") and response.action == "next":
        logger.info("Finished current batch‑sample")

logger.info(f"Total batch inference time after compile: {time.time() - start_time:.2f} seconds")
